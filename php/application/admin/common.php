 ...
 *
 */
function string2date($arr)
{
    $datas = array();
    foreach($arr as $vo){
        $json2arr= json_decode($vo,true);
        $json2arr["create_time"] = date('Y-m-d H:i',$json2arr["create_time"]);
        $datas[] =  (object)$json2arr;
    }
    $data = $datas;
    return $data;
}
 ...
 * 服务器地址+项目路径+图片路径
 */
function addSeverPath($str)
{
    $start= strripos($_SERVER['SCRIPT_NAME'],'/');
    $path=substr($_SERVER['SCRIPT_NAME'],0,$start+1);
